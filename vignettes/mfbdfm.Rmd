---
title: "mfbdfm: A Mixed-Frequency Bayesian Dynamic Factor Model"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{mfbdfm: A Mixed-Frequency Bayesian Dynamic Factor Model}
...
directly interpretable as that series' growth rate rather than a
generic activity index. The package ships a flagship application, the
**Weekly Activity Index (WAI)**, a high-frequency GDP indicator for
Switzerland anchored to GDP growth, which this vignette uses as the
worked example throughout. WAI is built to combine two kinds of data:
...
evaluation are described in Kronenberg (2026); see `?mfbdfm` for the
...
`ind_dfm()` estimates a single latent weekly factor $f_t$ whose
...
GDP growth closely by construction — see `?ind_dfm` for the details and
references, and `vignette("mfbdfm")`'s "Real-time GDP vintages" section
...
spreadsheet, a database query — `mfbdfm_data()` builds that structure
from a long or wide data frame, an `mts`, or a list of `ts`, and takes
the flow/stock classification from a `meta` table:
...
- Eckert, F., Kronenberg, P., Mikosch, H., & Neuwirth, S. (2025) —
  *Tracking economic activity with alternative high-frequency data*,
  ...
  `ind_dfm()` implements as a single-factor special case.
