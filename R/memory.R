...
#'
#' **Memory is linear in `q`, not quadratic -- and at long chains barely depends
#' on `q` at all.** The natural guess is that the sparse Cholesky of the
#' `q(t+s)` square factor precision matrix dominates, which would scale with
#' `q^2`. Measured, it does not: adding that term to the regression does not
#' improve it, because the `Gmat` working set is an order of magnitude larger.
#'
#' The `q` effect is also much weaker at long chains than at short ones -- each
#' extra factor costs about 156 MB at 30 retained draws but only about 29 MB at
#' 500. Same cause as the convexity below: once the evaluation phase binds, peak
#' memory is dominated by the retained draws and the per-series matrices built
#' from them, whose size is driven by `n * t`, while the `q`-dependent part of a
#' draw (`nq + pq^2 + t + s`) is negligible beside it. The model keeps `q` in the
#' `Gmat` term regardless, which makes it *loose* at high `q` and long chains
#' rather than wrong -- 1959 MB estimated against 1596 MB measured at `q = 4`,
#' 500 draws.
#'
...
#' # Calibration
#'
#' Fitted to eight fits measured one per fresh R process (the GC high-water mark
...
#' `MEM_SAFETY_FACTOR` so that it also covers the two long-chain points, which are
#' the settings the sweeps actually use. `q = 2` at 500 draws is the binding one
#' (covered at ratio 1.01); `q = 4` at 500 draws is covered with 23% to spare, for
