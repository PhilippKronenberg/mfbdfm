...
The sequence below is the whole user-facing workflow, using only
exported functions. `demo/run_package_demo.R` runs exactly this end to
end — install, fit, inspect, save — in about five minutes on the shipped
data, and is the quickest way to confirm an installation is sound:
...
Series go in as `ts` objects split into flows and stocks.
[`mfbdfm_data()`](https://philippkronenberg.github.io/mfbdfm/reference/mfbdfm_data.md)
builds that from a long or wide data frame, an `mts`, or a list of `ts`,
and — more usefully — makes the classification inspectable *before* a
...
Flow versus stock selects the temporal aggregation weights, so getting
it wrong changes results silently.
[`mfbdfm_data()`](https://philippkronenberg.github.io/mfbdfm/reference/mfbdfm_data.md)
requires `type` only where it can matter — below the highest frequency —
and puts weekly or daily series onto the 48-periods-per-year grid the
models are built on.
...
Two models, two entry points — **not one model with a `q` argument**.
They differ in identification, priors and how the dynamics are drawn:
...
ships with the package, so a real-time exercise needs no external data.
[`cut_data()`](https://philippkronenberg.github.io/mfbdfm/reference/cut_data.md)
trims every series to what was available at a given date, and the
`run_*()` wrappers fit one model at one evaluation date and write the
fit:
...
fixes the output layout, and the evaluation helpers work off it:
...
Frequency and date helpers used throughout:
